...
function cj()
{
	...
	$apicn = "maccmsapi-videolist-" . $rtype . "-" . $rpage . "-" . $rkey . "-" . $rday ;
	if (chkCache($apicn)){
		echo getCache($apicn);
		exit;
	}
	...
	if (!$rs){
		...
		while ($row = $db ->fetch_array($rs))
		{
		    ...
		    $typearr = getValueByArray($cache[0], "t_id", $row["d_type"]);
		  	//$vlink = "http://" . app_siteurl . $template->getVodLink($row["d_id"],$row["d_name"],$row["d_enname"],$row["d_type"],$typearr["t_name"],$typearr["t_enname"]);
			$plink = app_siteurl . $template->getVodPlayUrl($row["d_id"],$row["d_name"],$row["d_enname"],$row["d_type"],$typearr["t_name"],$typearr["t_enname"],1,1);
...
function vlist()
{
	...
	$sql = "select d_id,d_name,d_enname,d_type,d_time,d_remarks,d_playfrom from {pre}vod where 1=1 ";
	$sql1 = "select count(*) from {pre}vod where 1=1 ";
	
	if ($rtype > 0) { $where .= " and d_type=" . $rtype; }
	if (app_apivodfilter != "") { $where .= maccms_table_vod_filter." "; }
