...
class AppTemplate
{
    ...
    function typematrix()
    {
        ...
        for($i=0;$i<count($matches1[0]);$i++)
		{
            ...
            if($this->par_type=="letter"){
	            ...
	            $sql = "SELECT t_id,t_name,t_enname,t_key,t_des FROM {pre}" . $this->par_table . "_type WHERE t_hide=0 ";
				$where="";
				$orderstr="";
				$markhtml="";
    ...
    function playlist($id, $name, $enname, $typeid, $typename, $typeenname, $playfrom, $playserver, $playurl)
    {
    	global $mac;
    	$playurlarrlen=0;
    	$playfromarrlen=0;
        ...
        for($i=0;$i<count($matches1[0]);$i++)
		{
            ...
            for ($j=0;$j<$playfromarrlen;$j++){
                ...
                if ($playserverarrlen >= $j){
                    ...
                    $servertip = getVodXmlText("vodserver.xml","server", $playserverarr[$j] , 3);
                }
                ...
                $markhtml = replaceStr($markhtml, "[playlist:num]", $num);
    ...
    function loadvod($row,$typearr,$flag)
    {
        ...
        if ($flag == "play"){
            ...
            if (app_vodplayviewtype <3){
            	...
            	$this->replacePlayName($row["d_playfrom"],$row["d_playurl"],$mac["vodsrc"],$mac["vodnum"]);
            	$this->html = replaceStr($this->html, "[playinfo:info]", "<script>" . "\n" . getPlayInfo($row["d_name"], $row["d_playfrom"], $row["d_playserver"], $row["d_playurl"]) ."\n" . " </script>". "\n" );
            }
            else{
        ...
        if (!( $flag=="play" && app_vodplayviewtype>3)){
        	$this->playlist ($row["d_id"], $row["d_name"], $row["d_enname"], $row["d_type"], $this->page_typearr["t_name"], $this->page_typearr["t_enname"], $row["d_playfrom"], $row["d_playserver"], $row["d_playurl"]);
        }
        
        $this->downlist ($row["d_id"], $row["d_name"], $row["d_enname"], $row["d_type"], $this->page_typearr["t_name"], $this->page_typearr["t_enname"],  $row["d_downurl"]);
